    ...
    if (stringValue != null && !stringValue.equals(defaultValue) &&
      ...
      try {
        ...
        if (inputStream != null) {
          try {
            ...
            Log.d(TAG,"Failed to close InputStream."+ e.getMessage());
          }
        }
      }
      ...
      if (!FileManager.fileExistsAtPath(realPath)) {
        ...
        if (!FileManager.fileExistsAtPath(realPath)) {
          ...
          LPClassesMock.downloadFile(stringValue, urlValue, onComplete, onComplete);
 ...
 public static HashResults fileMD5HashCreateWithPath(InputStream is) {
  ...
  private static void enableResourceSyncing(List<Pattern> patternsToInclude,
    ...
    try {
      ...
      Log.d(TAG,"Error occurred when trying " + "to enable resource syncing." + e.getMessage());
    } finally {
      if (apk != null) {
        try {
          apk.close();
        } catch (IOException e) {
          Log.d(TAG,"Failed to close ZipInputStream."+ e.getMessage());
        }
      }
    }
    isInitialized = true;
  ...
  private static List<Pattern> compilePatterns(List<String> patterns) {
    ...
    for (String pattern : patterns) {
      try {
        ...
        Log.e(TAG,"Ignoring malformed resource syncing pattern: \"" + pattern +
  ...
  public static InputStream stream(boolean isResource, Boolean isAsset, Boolean valueIsInAssets,
    ...
    try {
      ...
      if (isResource && value.equals(defaultValue) && resourceData != null) {
        ...
        int resourceId = LPClassesMock.generateIdFromResourceName(value);
      ...
      Log.d(TAG,"Failed to load a stream." + e.getMessage());
      return null;
    }
  }
}
