...
public class AESCrypt {
  ...
  public String encrypt(String plaintext) {
    ...
    if (appId == null || appId.isEmpty()) {
      Log.e(TAG,"Encrypt called with null appId.");
      return null;
    }
    ...
    if (cipherText.isEmpty() || cipherText.equals(plaintext) || !cipherText.startsWith("[")) {
      Log.d(TAG,"Invalid ciphertext: " + cipherText);
      return null;
    }
  ...
  public String decrypt(String cipherText) {
    ...
    if (encryptionSpec == null) {
      Log.d(TAG,"Got null encryptionSpec for encrypted: " + cipherText);
    } else {
      switch (encryptionSpec.first) {
        case LEGACY_TOKEN:
          if (token == null || token.isEmpty()) {
            Log.e(TAG,"Decrypt called with null token.");
          } else {
            ...
            if (result != null && parseCiphertextInternal(result) != null) {
              Log.e(TAG,"Discarding legacy value that appears to be an encrypted value: " +
                  result);
              return null;
            }
          }
        ...
        case APP_ID_KEY:
          if (appId == null || appId.isEmpty()) {
            Log.e(TAG,"Decrypt called with null appId.");
          } else {
    ...
    if (result == null) {
      Log.d(TAG,"Unable to decrypt " + cipherText);
    }
    return result;
  }
  ...
  public static String encryptInternal(String password, String plaintext) {
    try {
      ...
      Log.d(TAG,"Unable to encrypt " + plaintext, e);
      return null;
    }
  }
  ...
  private static String decryptInternal(String password, String ciphertext) {
    try {
      ...
      Log.d(TAG,"Could not encode UTF8 string.\n" + Log.getStackTraceString(e));
    }
    return null;
  }
