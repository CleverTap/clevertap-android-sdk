...
class EventQueueManagerTest : BaseTestCase() {
    ...
    fun test_queueEvent_for_profileEvent_will_process_further_and_add_to_queue_when_event_should_not_be_dropped() {
        ...
        verify { inAppController.onQueueProfileEvent(any(), any()) }
        verify { corestate.sessionManager.lazyCreateSession(application) }
        verify { eventQueueManager.pushInitialEventsAsync() }
    ...
    fun test_flushQueueSync_when_net_is_offline() {
        ...
        verifyOrder {
            corestate.callbackManager.invokeCallbacksForNetworkError(ctVariables)
            corestate.callbackManager.invokeBatchListener(JSONArray(), false)
        }
        verify(exactly = 0) {
    ...
    fun test_processEvent_when_type_is_ping_event() {
        ...
        mockkStatic(Utils::class) {
            ...
            assertEquals(
                ...
                actualEvent.getJSONObject(Constants.ERROR_KEY)["c"]
            )
            assertEquals(
                validationResult.errorDesc,
                actualEvent.getJSONObject(Constants.ERROR_KEY)["d"]
            )
