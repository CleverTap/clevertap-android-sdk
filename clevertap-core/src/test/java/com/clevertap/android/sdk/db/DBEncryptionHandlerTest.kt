...
internal class DBEncryptionHandlerTest {
    ...
    fun `unwrapDbData - null data does not call decryptSafe`() {
        ...
        verify(exactly = 0) { cryptHandler.decryptSafe(any(), any()) }
    ...
    fun `wrapDbData - wraps data with encryption only for encryptionLevel = FULL_DATA`() {
        ...
        every { cryptHandler.encryptSafe(any(), any()) } returns encryptedText
        ...
        verify(exactly = 0) { cryptHandler.encryptSafe(any(), any()) }
        ...
        verify(exactly = 1) { cryptHandler.encryptSafe(plainText, EncryptionAlgorithm.AES_GCM) }
    }
}